...
import { CircleDollarSign, Plus, Search, Edit, Trash2, MoreHorizontal, Loader2 } from "lucide-react";
...
interface CostCenter { id: string; code: string; name: string; departmentId: string | null; budgetApplicable: boolean; isActive: boolean; department?: { name: string; code: string } | null; }
...
export default function CostCenterPage() {
    ...
    const [form, setForm] = useState({ code: "", name: "", departmentId: "", budgetApplicable: true });

    const fetchData = async () => {
        try {
            const [ccRes, deptRes] = await Promise.all([fetch("/api/master/cost-center"), fetch("/api/master/department")]);
            if (ccRes.ok) setItems(await ccRes.json());
            if (deptRes.ok) setDepartments(await deptRes.json());
        } catch { toast.error("Failed to fetch"); } finally { setLoading(false); }
    };
    useEffect(() => { fetchData(); }, []);

    const openCreate = () => { setEditing(null); setForm({ code: "", name: "", departmentId: "", budgetApplicable: true }); setDialogOpen(true); };
    const openEdit = (item: CostCenter) => { setEditing(item); setForm({ code: item.code, name: item.name, departmentId: item.departmentId || "", budgetApplicable: item.budgetApplicable }); setDialogOpen(true); };

    const handleSave = async () => {
        if (!form.code || !form.name) { toast.error("Code and Name are required"); return; }
        setSaving(true);
        try {
            const url = editing ? `/api/master/cost-center/${editing.id}` : "/api/master/cost-center";
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/master/cost-center/${deleting.id}`, { method: "DELETE" });
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-white flex items-center gap-2"><CircleDollarSign className="h-6 w-6 text-blue-400" />Cost Center</h2>
                    <p className="text-sm text-slate-400 mt-1">Manage cost centers for budget tracking</p>
                </div>
                <Button onClick={openCreate} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white"><Plus className="h-4 w-4 mr-2" />Add Cost Center</Button>
            </div>
            ...
            <div className="bg-[#111827]/80 border border-white/[0.06] rounded-xl overflow-hidden">
                <table className="w-full">
                    <thead><tr className="border-b border-white/[0.06]">
                        ...
                        <th className="text-left px-5 py-3.5 text-xs font-medium text-slate-400 uppercase tracking-wider">Department</th>
                        <th className="text-left px-5 py-3.5 text-xs font-medium text-slate-400 uppercase tracking-wider">Budget</th>
                    ...
                    <tbody className="divide-y divide-white/[0.04]">
                        {loading ? <tr><td colSpan={6} className="text-center py-12"><Loader2 className="h-6 w-6 animate-spin text-blue-400 mx-auto" /></td></tr>
                            : filtered.length === 0 ? <tr><td colSpan={6} className="text-center py-12 text-slate-400">No cost centers found</td></tr>
                                : filtered.map(item => (
                                    <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="px-5 py-3.5 text-sm text-slate-300">{item.department ? `${item.department.code} - ${item.department.name}` : "-"}</td>
                                        <td className="px-5 py-3.5"><Badge className={item.budgetApplicable ? "bg-blue-500/10 text-blue-400" : "bg-slate-500/10 text-slate-400"}>{item.budgetApplicable ? "Yes" : "No"}</Badge></td>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="bg-[#1e293b] border-white/[0.08] text-white max-w-md">
                    <DialogHeader><DialogTitle>{editing ? "Edit Cost Center" : "Create Cost Center"}</DialogTitle><DialogDescription className="text-slate-400">Cost centers track expenses per department.</DialogDescription></DialogHeader>
                    <div className="space-y-4 py-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={form.departmentId} onValueChange={(v) => setForm({ ...form, departmentId: v === "none" ? "" : v })}>
                                <SelectTrigger className="bg-[#0d1117] border-white/[0.08] text-white"><SelectValue placeholder="Select department" /></SelectTrigger>
                        ...
                        <div className="flex items-center gap-2">
                            <Checkbox id="budget" checked={form.budgetApplicable} onCheckedChange={(v) => setForm({ ...form, budgetApplicable: !!v })} className="border-white/20" />
                            <Label htmlFor="budget" className="text-slate-300 text-sm">Budget Applicable</Label>
                        </div>
                    </div>
                    <DialogFooter>
            ...
            <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <DialogContent className="bg-[#1e293b] border-white/[0.08] text-white max-w-sm">
                    <DialogHeader><DialogTitle>Delete Cost Center</DialogTitle><DialogDescription className="text-slate-400">Delete <span className="text-white font-medium">{deleting?.name}</span>?</DialogDescription></DialogHeader>
                    <DialogFooter>
